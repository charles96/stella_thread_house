...
# AI
AI_REASONING_ENDPOINT=https://api.openai.com/v1   # OpenAI 호환 base URL (/v1 포함)
AI_REASONING_API_KEY=sk-xxxxxxxx                  # 인증 불필요한 Local LLM 서버면 비워둠
AI_REASONING_MODEL=gpt-4o                         # reasoning 모델
AI_VISION_ENDPOINT=https://api.openai.com/v1      # vision 그룹 — 다른 공급자로 지정 가능
AI_VISION_API_KEY=sk-xxxxxxxx
AI_VISION_MODEL=gpt-4o                            # vision 모델
...
services:
  postgres:
    image: pgvector/pgvector:pg18   # PostgreSQL 18 + pgvector (vector 확장에 필요)
    environment:                    # .env 에서 주입 (기본값 표시)
...
# AI — 그룹별 설정: Reasoning(텍스트 답변) / Vision(이미지 분석)
# 각 그룹은 OpenAI 호환 endpoint(/v1 포함)·API 키·모델·최대 출력 토큰을 가진다.
# 최초 부팅 시 DB에 시드되며, 이후에는 Settings → AI 값이 우선한다.
#   OpenAI 클라우드: https://api.openai.com/v1        (예: 모델 gpt-4o)
#   Anthropic:      https://api.anthropic.com/v1      (예: 모델 claude-...)
#   로컬 런타임(Ollama/vLLM/LM Studio 등): http://host.docker.internal:11434/v1
AI_REASONING_ENDPOINT=https://api.openai.com/v1
AI_REASONING_API_KEY=sk-xxxxxxxxxxxxxxxxxxxxx   # 로컬 서버(인증 불필요)면 비워둠
AI_REASONING_MODEL=gpt-4o
AI_REASONING_MAX_TOKENS=16384
AI_VISION_ENDPOINT=https://api.openai.com/v1    # 다른 공급자/모델로 지정 가능
AI_VISION_API_KEY=sk-xxxxxxxxxxxxxxxxxxxxx
AI_VISION_MODEL=gpt-4o                          # vision(멀티모달) 모델
